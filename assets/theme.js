...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  var ProductVariants = function () {
    function ProductVariants(container, options) {
      ...
      this.variantsData = jsonData['productVariants'];
      this.variantsInventories = jsonData['inventories'] || {};
      this.masterSelector = this.element.querySelector('#product-select-' + this.productData['id']);

      // We init value with the first selected variant
      this.variantsData.forEach(function (variant) {
    ...
    _createClass(ProductVariants, [{
      ...
      value: function _updateAddToCartButton(newVariant) {
        ...
        var newVariantOption = this.element.querySelector('[name="id"] option[value="' + this.currentVariant.id + '"]')
        var preorder = newVariantOption.dataset.inventoryPolicy === 'continue' && Number(newVariantOption.dataset.inventoryQuantity) < 1
        var buttonText = window.languages.productFormAddToCart
        var preorderLabel = this.element.querySelector('.preorder-info-text')

        if(preorder) {
          buttonText = window.languages.productFormPreorder
          preorderLabel.classList.remove('js-hidden')
        } else {
          preorderLabel.classList.add('js-hidden')
        }
        ...
        if (!newVariant) {
          ...
          if (newVariant['available']) {
            ...
            if (undefined === this.options['showPriceInButton'] || this.options['showPriceInButton']) {
              ...
              addToCartButton.innerHTML = '<span>' + buttonText + '</span>';
            }
          } else {
            addToCartButton.setAttribute('disabled', 'disabled');
            addToCartButton.classList.add('Button--secondary');
      ...
      value: function _onOptionChanged(newValue, target, activator) {
        console.log('onOptionChanged', newValue, target, activator)
        ...
        if (activator && activator.closest('.js-form-option-engraving')) {
          var engravingContainer = this.element.querySelector('.js-engraving-container')
          var engravingInput = engravingContainer.querySelector('.js-engraving-text')
          var engravingFontProperty = engravingContainer.querySelector('.js-engraving-font')

          engravingInput.setAttribute('data-font', newValue);
          engravingFontProperty.value = newValue;
        }
        ...
        if (this.currentVariant) {
          ...
          var engravingContainer = this.element.querySelector('.js-engraving-container');
          var formOptionEngraving = this.element.querySelector('.js-form-option-engraving');

          if (engravingContainer && formOptionEngraving) {
            var engraved = this.element.querySelector('.ProductForm__Option--Engraving .ProductForm__SelectedValue').innerHTML === 'Yes';

            if (engraved) {
              engravingContainer.classList.add('js-active');
              formOptionEngraving.classList.add('js-active');
            } else {
              engravingContainer.classList.remove('js-active');
              formOptionEngraving.classList.remove('js-active');
            }
          }
      ...
      value: function _addToCart(event) {
        ...
        // First we handle engraving properties
        var formElement = this.element.querySelector('form[action*="/cart/add"]');
        var formSerialize = __WEBPACK_IMPORTED_MODULE_2__helper_Form__["default"].serialize(formElement);
        
        if (this.element.querySelector('.ProductForm__Option--Engraving .ProductForm__SelectedValue')) {
            var productEngraved = this.element.querySelector('.ProductForm__Option--Engraving .ProductForm__SelectedValue').innerHTML === 'Yes';
            var engravingText = formElement.querySelector('.js-engraving-text');

            if (productEngraved && engravingText && engravingText.value.length === 0) {
              formElement.querySelector('.js-engraving-error').style.display = 'block';

              return false;
            }

            if (!productEngraved) {
              formSerialize.properties = null
            }
        }

        var addToCartButton = this.element.querySelector('.ProductForm__AddToCart');

        // Then we switch the status of the button
        addToCartButton.setAttribute('disabled', 'disabled');
    ...
    _createClass(CollectionSection, [{
      ...
      value: function _toggleTag(event) {
        ...
        if (element.classList.contains('is-active')) {
          ...
          if (!container) {
            container = element.closest('.my-heart-filters')
          }

          var activeSibling = container.querySelector('.is-active');
        ...
        console.log(this.temporaryTags);
      ...
      value: function _updateActiveTags() {
        ...
        __WEBPACK_IMPORTED_MODULE_3__helper_Dom__["default"].nodeListToArray(this.element.querySelectorAll('.CollectionFilters [data-tag], .my-heart-filters [data-tag]')).forEach(function (item) {
...
/***/function (module, __webpack_exports__, __webpack_require__) {
  ...
  var SlideshowSection = function () {
    function SlideshowSection(container) {
      ...
      var sliderOptions = JSON.parse(this.element.querySelector('[data-flickity-config]').dataset.flickityConfig)
      var slidesToShow = Number(this.element.dataset.slidesToShow)
      var numberOfSlides = this.element.querySelectorAll('.Carousel__Cell').length

      if (window.matchMedia("(max-width: 1200px)").matches) {
        slidesToShow = Math.min(numberOfSlides, slidesToShow, 3)
      }

      if (window.matchMedia("(max-width: 1024px)").matches) {
        slidesToShow = Math.min(numberOfSlides, slidesToShow, 2)
      }

      if (window.matchMedia("(max-width: 640px)").matches) {
        slidesToShow = Math.min(numberOfSlides, slidesToShow, 1)
      }

      sliderOptions.draggable =
      sliderOptions.wrapAround =
      sliderOptions.pageDots = numberOfSlides > slidesToShow

      this.element.querySelectorAll('.Carousel__Cell').forEach(cell => {
        cell.style.width = 100 / slidesToShow + '%';
      })

      this.element.querySelector('[data-flickity-config]').dataset.flickityConfig = JSON.stringify(sliderOptions)
      ...
      if (this.slideshow.flickityInstance.cells.length > 0) {
        this.slideshow.flickityInstance.cells.forEach((cell) => {
          this._transitionToSlide(cell.element, true);
        })
      }
